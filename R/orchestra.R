...
as.character.orchestra <- function(orchestra){
  ...
  names_equal <- c("0dbfs", "kr", "ksmps", "nchnls", "sr")
  names_equal <- intersect(names_equal, names(orchestra))
  result <- paste(names_equal, unlist(orchestra[names_equal]), sep = " = ")
  
  paste(c(result, as.character.instrument(orchestra$instrument)), collapse = "\n")
}